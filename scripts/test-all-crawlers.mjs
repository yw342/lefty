 ...
 * (뮬 테스트 시 로컬에서 SCRAPER_API_KEY 설정 필요)
 */
...
async function main() {
  ...
  console.log('\n--- 요약 ---');
  console.log('중고나라:', joongna.length, '| 뮬:', mule.length, '| 당근:', daangn.length);
  const total = joongna.length + mule.length + daangn.length;
  if (total === 0) {
    console.log('수집된 매물 없음. 뮬은 SCRAPER_API_KEY 설정 시 재시도.');
    process.exitCode = 1;
  } else {
